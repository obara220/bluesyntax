package com.panda.aoodds.esport.filter;
        ...
        if (appResponse.hasException() && GenericService.class != invoker.getInterface()) {
            try {
                ...
                if (exception instanceof ApiException) {
                    appResponse.setException(new RpcException(exception.getMessage()));
                    return;
                }
                try {
                ...
                log.info("DubboExceptionFilter ,Got unchecked and undeclared exception which called by " + RpcContext.getContext().getRemoteHost() + ". service: " + invoker.getInterface().getName() + ", method: " + invocation.getMethodName() + ", exception: " + exception.getClass().getName() + ": " + exception.getMessage(), exception);
